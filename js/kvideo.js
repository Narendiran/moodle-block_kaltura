...
//    var popup = document.getElementById;
    ...
    objModalboxContent.style.marginTop = '0px';
    objModalboxContent.style.marginBottom = '0px';
    objModalboxContent.style.marginLeft = '0px';
    objModalboxContent.style.marginRight = '0px';
    objModalboxContent.style.paddingTop = '0px';
    objModalboxContent.style.paddingBottom = '0px';
    objModalboxContent.style.paddingLeft = '0px';
    objModalboxContent.style.paddingRight = '0px';


    // Get margin, border and padding information from the 'page' popup
    var divpage         = document.getElementById("page");
    var pagepadleft     = getStyle(divpage, "padding-left");
    var pagemarginleft  = getStyle(divpage, "margin-left");
    var pageborderleft  = getStyle(divpage, "border-left-width");


    var pagepadtop      = getStyle(divpage, "padding-top");
    var pagemargintop   = getStyle(divpage, "margin-top");
    var pagebordertop   = getStyle(divpage, "border-top-width");
  
    pagepadleft     = parseInt(pagepadleft.slice(0, 2));
  
    switch (pagemarginleft) {
      case 'auto': // IE8 does not return the default value in px.
        pagemarginleft  = '0px';
      default:
        pagemarginleft  = parseInt(pagemarginleft.slice(0, 2));
        break;
    }
  
    switch (pageborderleft) {
      case 'thin': // IE8 does not return the default value in px.
      case 'medium':
      case 'thick':
        pageborderleft  = '0px';
      default:
        pageborderleft  = parseInt(pageborderleft.slice(0, 2));
        break;
    }

    pagepadtop      = parseInt(pagepadtop.slice(0, 2));
  
    switch (pagemargintop) {
      case 'auto': // IE8 does not return the default value in px.
        pagemargintop   = '0px';
      default:
        pagemargintop   = parseInt(pagemargintop.slice(0, 2));
        break;
    }
  
    switch (pagebordertop) {
      case 'thin': // IE8 does not return the default value in px.
      case 'medium':
      case 'thick':
        pagebordertop   = '0px';
      default:
        pagebordertop   = parseInt(pagebordertop.slice(0, 2));
        break;
     }
  
    var iframeheight = pagepadtop + pagemargintop + pagebordertop + parseInt(height);
    var iframewidth  = parseInt(width);

    if ( url != null ) {
        thehtml = '<iframe allowtransparency="true" name="'+tmp_id+'" id="kaltura_modal_iframe" scrolling="no" width="' + iframewidth + '" height="' + iframeheight + '" frameborder="0" src="' + url + '">';
...
function kalturaCloseModalBox () {
    ...
    /* Fixes KMI-75 - IE8 bug.  Must specifically remove body tag within iframe
     * otherwise body tag will remain on the page.
     */
    if (iframe_obj.contentDocument) {
        kcwdiv = iframe_obj.contentDocument.getElementById("blocks-kaltura-kcw");
    } else {
        kcwdiv = iframe_obj.contentWindow.document.getElementById("blocks-kaltura-kcw");
    }

    if (kcwdiv) {
        kcwdiv.parentNode.removeChild( kcwdiv );
    }
...
function get_field_obj(field_name) {
    if (this != window.top) {
        return window.top.get_field_obj(field_name);
    }
...
function switch_item_in_player(entry_id, type) {
    ...
    if (!type || type == 2) {
        ...
        $('#kplayer').hide();
        thumb_url = $.ajax(
            {url: wwwroot + "/blocks/kaltura/kthumb_url.php?id="+entry_id+"&type=1&height=364&width=410",
...
function openLightBox(entryId,objId) {
    ...
    //else
        ...
        url = wwwroot + "/blocks/kaltura/kdp.php?entry="+entryId+"&seekto="+time;
...
function replace_resource_video(tag,vid_id) {
    ...
    var action = 'video';
    var url    = wwwroot + "/blocks/kaltura/html_resource_ajax/replace_tags.php";
